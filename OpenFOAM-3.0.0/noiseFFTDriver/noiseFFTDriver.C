...
Foam::scalar Foam::noiseFFTDriver::p0 = 2e-5;


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::noiseFFTDriver::noiseFFTDriver
(
    const scalar deltat,
...
Foam::noiseFFTDriver::noiseFFTDriver(const fileName& pFileName, const label skip)
:
    scalarField(),
    deltat_(0.0)
{
    ...
    if (!pFile.good())
    {
        FatalErrorIn("libnoiseFFT::libnoiseFFT(const scalar, const scalarField&)")
    ...
    if (skip)
    {
        ...
        for (label i=0; i<skip; i++)
        {
            ...
            if (!pFile.good() || pFile.eof())
            {
                FatalErrorIn
                (
                    "libnoiseFFT::libnoiseFFT(const scalar, const scalarField&)"
                )
...
Foam::graph Foam::noiseFFTDriver::pt(const scalarField t) const
{
//    scalarField t(size());
//    forAll(t, i)
//    {
//        t[i] = startTime_+ i*deltat_*n_;
//    }
...
Foam::tmp<Foam::scalarField> Foam::noiseFFTDriver::window
(
    const label N,
    const label ni
) const
{
    ...
    {
        FatalErrorIn("libnoiseFFT::window(const label, const label) const")
...
Foam::tmp<Foam::scalarField> Foam::noiseFFTDriver::Hanning(const scalarField t, const label N) const
{
//    scalarField t(N);
//    forAll(t, i)
//    {
...
Foam::tmp<Foam::scalarField> Foam::noiseFFTDriver::Pf
(
...
Foam::graph Foam::noiseFFTDriver::meanPf
(
    const scalarField t,
    const label N,
    const label nw
) const
{
    if (N > size())
    {
        FatalErrorIn("libnoiseFFT::meanPf(const label, const label) const")
...
Foam::graph Foam::noiseFFTDriver::RMSmeanPf
(
    const scalarField t,
    const label N,
    const label nw
) const
{
    if (N > size())
    {
        FatalErrorIn("libnoiseFFT::RMSmeanPf(const label, const label) const")
            << "Requested window is outside set of data" << endl
            << "number of data = " << size() << endl
            << "size of window = " << N << endl
            << "window  = " << nw
...
Foam::graph Foam::noiseFFTDriver::psd(const graph& psdPf) const
{
    return graph
    (
        "PSD(f)",
        ...
        psdPf.x(),
        psdPf.y()*psdPf.y()/1
//        20*log10(psdPf.y()*psdPf.y()/p0)
    );
}

Foam::graph Foam::noiseFFTDriver::spl(const graph& splPf) const
{
    return graph
    (
        "SPL(f)",
...
Foam::graph Foam::noiseFFTDriver::Ldelta
(
    const graph& gLf,
    const scalar f1,
    const scalar fU
...
Foam::graph Foam::noiseFFTDriver::Pdelta
(
    const graph& gPf,
    const scalar f1,
    const scalar fU
...
Foam::scalar Foam::noiseFFTDriver::Lsum(const graph& gLf) const
{
    const scalarField& Lf = gLf.y();
...
Foam::scalar Foam::noiseFFTDriver::dbToPa(const scalar db) const
{
    return p0*pow(10.0, db/20.0);
}


Foam::tmp<Foam::scalarField> Foam::noiseFFTDriver::dbToPa
(
