...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
TEST_CASE("Checks functionality in the libAcoustics", "[testDipoleSource]")
{
    ...
    SECTION ("Dipole")
    {
        ...
        while (timeObj.run())
        {
            ...
            forAll(fwh.getSoundObservers(), iObs)
            {
                ...
                scalar cosTet = fwh.getSoundObservers()[iObs].position()[0]/rSource;
                analyticResult[iObs].append(r0*r0*r0*c0*k*U0*rho*cosTet*(k*(-2.0*rSource +r0*(2.0+k*k*r0*rSource))*Foam::cos(k*(r0-rSource+c0*t)) + (-2.0 + k*k*r0*(r0-2.0*rSource))*Foam::sin(k*(r0-rSource+c0*t)) )/((4.0 + r0*r0*r0*r0*k*k*k*k)*rSource*rSource));
            }
        }
        ...
        while (timeObj.run())
        {
            ...
            forAll(fwh.getSoundObservers(), iObs)
            {
                forAll(Sf,iFace)
                {
                    ...
                    pS[iFace] = r0*r0*r0*c0*k*U0*rho*cosTet*(k*(-2.0*r0 + r0*(2.0+k*k*r0*r0))*Foam::cos(k*(r0-r0+c0*t)) + (-2.0 + k*k*r0*(r0-2.0*r0))*Foam::sin(k*(r0-r0+c0*t)) ) / ((4.0 + r0*r0*r0*r0*k*k*k*k)*r0*r0);
                    vector normals = Sf[iFace]/magSf[iFace];
                    uS[iFace] = (r0*r0*r0*U0*cosTet*((4.0 + k*k*(4.0*r0*r0-2.0*r0*r0+r0*r0*(-2.0 + k*k*r0*r0)))*Foam::cos(k*(r0-r0 + c0*t)) + 2.0*k*(r0-r0)*(2.0 +k*k*r0*r0)*Foam::sin(k*(r0-r0+c0*t)))/((4.0 + r0*r0*r0*r0*k*k*k*k)*r0*r0*r0))*normals;
                }
        ...
        forAll(fwh.getSoundObservers(), iObs)
        {
            ...
            forAll(oap[iObs], pI)
            {
                if (pI >= 1000)
                {
                    REQUIRE ((oap[iObs][pI]-analyticResult[iObs][pI]) <= 0.05);
                }
            }
        }
    }
